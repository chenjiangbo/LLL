...
**6.** 回测模式，如果订阅标的行情日线 frequency='1d' 或 不订阅行情，直接调用 current ，会根据 run() 函数指定的 backtest_intraday 参数返回：（python 版本 >= 3.0.178起）
...
**返回值:参考[level2 逐笔委托数据](/docs2/sdk/python/数据结构.html#L2OrderLevel2-逐笔委托)**
